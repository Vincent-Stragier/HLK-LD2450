...
def query_target_tracking(ser: serial.Serial) -> int:
    ...
    if not _get_command_success(response):
...
def set_zone_filtering(
    ...
    command_value = bytes.fromhex(
        f"{zone_filtering_mode:04x} {region1_x1:04x} {region1_y1:04x} "
        f"{region1_x2:04x} {region1_y2:04x} {region2_x1:04x} "
        f"{region2_y1:04x} {region2_x2:04x} {region2_y2:04x} "
        f"{region3_x1:04x} {region3_y1:04x} {region3_x2:04x} {region3_y2:04x}"
    )
